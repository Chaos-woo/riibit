...
def fetch_hupu():
    ...
    if response.status_code == 200:
        ...
        # 查找所有文章列表项
        list_dom = soup.find_all('div', attrs={'class': 'list-item-wrap'})

        # 提取列表数据
        list_data = []
        for item in list_dom:
            # 提取信息
            t_info = item.find('div', attrs={'class':'t-info'})
            if(t_info == None):
                continue
            a = t_info.find('a')
            url = a.get('href') if a else None
            title = a.find('span', attrs={'class': 't-title'}).get_text(strip=True) if a else ''

            # 构建移动端URL
            hot_search_url = f'https://bbs.hupu.com{url}'
