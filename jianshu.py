...
def fetch_jianshu():
    ...
    if response.status_code == 200:
        ...
        # 查找所有文章列表项
        list_dom = soup.find_all('ul', attrs={'class':'note-list'})

        # 提取列表数据
        list_data = []
        for ul in list_dom:
            li_dom = ul.find_all('li')
            for item in li_dom:
                dom = BeautifulSoup(str(item), 'html.parser')  # 将item转换为BeautifulSoup对象
                href = dom.find('a').get('href', '')  # 获取href属性
                title = dom.find('a', attrs={'class':'title'}).text.strip() if dom.find('a', attrs={'class':'title'}) else ''  # 获取标题并去除空白字符

                # 构建完整的文章URL
                hot_search_url = f'https://www.jianshu.com{href}'
                list_data.append({
                    'title': title,
                    'url': hot_search_url,
                })
