...
def fetch_52pojie():
    ...
    url2 = 'https://www.52pojie.cn/forum.php?mod=forumdisplay&fid=5&filter=author&orderby=dateline'
    ...
    if response.status_code == 200:
        ...
        # 查找所有文章列表项
        list_dom = soup.find_all('tbody', attrs={'id': re.compile(r'^normalthread_\d+')})

        # 提取列表数据
        list_data = []
        for item in list_dom:
            # 提取信息
            a = item.find('a', attrs={'class': 'xst'})
            url = a.get('href')
            title = a.get_text(strip=True)

            # 构建移动端URL
            hot_search_url = f'https://www.52pojie.cn/{url}'
    ...
    if response.status_code == 200:
        ...
        for item in list_dom:
            # 提取信息
            a = item.find('a', attrs={'class': 's xst'})
            url = a.get('href')
            title = a.get_text(strip=True)
