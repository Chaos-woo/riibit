...
def fetch_xueqiu():
    ...
    response = session.get('https://xueqiu.com/snowman/login', headers=headers)
    # 热搜地址
    next_id = -1
    url = f'https://xueqiu.com/v4/statuses/public_timeline_by_category.json?since_id=-1&max_id={next_id}&count=10&category=-1'
    ...
    if response.status_code == 200:
        ...
        if response.status_code == 200:
            data = response.json()
            hot_searches = hot_searches + data['list']
        ...
        for item in hot_searches:
            # 构建超链接文本和链接
            data_str_json = json.loads(item['data'])

            link_text = data_str_json['title']
            link = f'https://xueqiu.com{data_str_json["target"]}'
