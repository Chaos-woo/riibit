...
def fetch_douban_group():
    ...
    if response.status_code == 200:
        ...
        # 查找所有文章列表项
        list_dom = soup.find_all('div', attrs={'class': 'channel-item'})

        # 提取列表数据
        list_data = []
        for item in list_dom:
            # 提取信息
            h3_a = item.find('h3').find('a')
            url = h3_a.get('href') if h3_a else None
            title = h3_a.text.strip() if h3_a else ''
            id = get_numbers(url)

            # 构建移动端URL
            hot_search_url = f'https://m.douban.com/group/topic/{id}/' if id else ''
