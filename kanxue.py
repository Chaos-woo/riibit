...
def fetch_kanxue():
    ...
    if response.status_code == 200:
        ...
        # 查找所有文章列表项
        list_dom = soup.find_all('div', attrs={'class': 'col-md-4 pl-0'})

        # 获取到新帖
        for item in list_dom:
            div = item.find('div', attrs={'class': 'card-title pb-1 font-weight-bold'})
            a = div.find('a')
            a_text = a.get_text(strip=True)
            if (a_text.find('新') != -1):
                list_dom = item.find_all('a', attrs={'class': 'bbs_home_page_list_title'})
                break
        ...
        for item in list_dom:
            ...
            # 构建移动端URL
            hot_search_url = f'https://bbs.kanxue.com/{url}'
